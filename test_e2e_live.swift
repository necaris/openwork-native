...
p.arguments = ["opencode", "serve", "--port", "8778"]
try! p.run()
sleep(2)

let url = URL(string: "http://localhost:8778/event")!
    ...
    for try await byte in bytes {
        ...
        if char == "\n" {
            if currentLine.isEmpty {
                print("FLUSHING EVENT, pending lines: \(pendingLines.count)")
...
Task {
    sleep(1)
    var postReq = URLRequest(url: URL(string: "http://localhost:8778/session")!)
    ...
    sleep(1)
    var promptReq = URLRequest(url: URL(string: "http://localhost:8778/session/\(sessionID)/prompt_async")!)
