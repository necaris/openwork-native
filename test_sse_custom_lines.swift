...
class MockState {
    func run() async {
        ...
        p.arguments = ["opencode", "serve", "--port", "8776"]
        try! p.run()
        sleep(2)

        let url = URL(string: "http://localhost:8776/event")!
        ...
        sleep(1)
        var postReq = URLRequest(url: URL(string: "http://localhost:8776/session")!)
        ...
        sleep(1)
        var promptReq = URLRequest(url: URL(string: "http://localhost:8776/session/\(sessionID)/prompt_async")!)
