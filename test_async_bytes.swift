...
class MockState {
    func run() async {
        ...
        p.arguments = ["opencode", "serve", "--port", "8777"]
        try! p.run()
        sleep(2)

        let url = URL(string: "http://localhost:8777/event")!
        ...
        sleep(1)
        var postReq = URLRequest(url: URL(string: "http://localhost:8777/session")!)
        ...
        sleep(1)
        var promptReq = URLRequest(url: URL(string: "http://localhost:8777/session/\(sessionID)/prompt_async")!)
